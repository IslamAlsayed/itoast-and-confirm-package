...
if (!function_exists('isIToastArray')) {
    function isIToastArray($type, $key = null)
    {
        if (!$key)
...
if (!function_exists('shakingOrNot')) {
    function shakingOrNot($type)
    {
        $shakeClass = '';

        if ($type === 'confirm') {
            $shakeClass = config('itoast.confirm_shake') ? 'shaking confirm-forever' : 'confirm-forever';
        } elseif (config('itoast.itoast_shake')) {
            $shakeClass = 'shaking';
        }
