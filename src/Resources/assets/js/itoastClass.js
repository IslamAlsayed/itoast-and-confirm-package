export const Itoast = {
    ...
    resetTimes() {
        ...
        this.root.style.setProperty(
            "--itoast-visible-time",
            this.parseTime(configItoast.visible_time || "7s")
        );
        this.root.style.setProperty(
            "--itoast-exit-time",
            configItoast.exit_time || "0.3s"
        );
        this.root.style.setProperty(
            "--itoast-start-delay-time",
            configItoast.start_delay_time || "1s"
        );
    },
    ...
    show(options) {
        ...
        if (
            (options.confirm && configItoast.confirm_shake) ||
            configItoast.itoast_shake
        ) {
            itoast.classList.add("shaking", "confirm-forever");
        }

        if (
            (options.confirm && configItoast.confirm_pin) ||
            configItoast.itoast_pin ||
