...
export function resetItoastTimes() {
  root.style.setProperty("--itoast-move", "enable");
  root.style.setProperty("--itoast-enter-time", "0.5s");
  root.style.setProperty("--itoast-visible-time", parseIToastTime("7s"));
  root.style.setProperty("--itoast-exit-time", "0.5s");
  root.style.setProperty("--itoast-start-delay-time", "1s");
}

export function setItoastTimes(itoast) {
  const baseVisibleTime = parseIToastTime("7s");
  const enterTime = parseIToastTime("0.5s");
  const exitTime = parseIToastTime("0.5s");
  const startDelayTime = parseIToastTime("1s");
...
export function pushItoastConfirm({
  ...
  if (pin == "pin" || itoast.classList.contains("confirm-forever")) {
    const pin = document.createElement("i");
    pin.className = "itoast-icon pin fas fa-thumbtack";
