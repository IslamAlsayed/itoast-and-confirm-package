...
document.addEventListener("click", function (e) {
  ...
  if (toastBtn) {
    const type = toastBtn.dataset.type || toastBtn.dataset.theme || "warn";
    const title = toastBtn.dataset.title || type;
    const message = toastBtn.dataset.message || "default message" || null;
  ...
  if (confirmBtn) {
    ...
    const type = confirmBtn.dataset.type || confirmBtn.dataset.theme || "warn";
    const title = confirmBtn.dataset.title || "confirmation" || type;
    const message = confirmBtn.dataset.message || "You are sure?" || null;
    const pin = confirmBtn.dataset.pin || null;
    const emoji = confirmBtn.dataset.emoji || null;
    const icon = confirmBtn.dataset.icon || null;
    const onconfirm = confirmBtn.dataset.onconfirm || "Yes";
    const onconfirmLink = confirmBtn.dataset.onconfirmlink || "#";
    const oncancel = confirmBtn.dataset.oncancel || "No";
